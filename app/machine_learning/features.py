def build_features(row):
    ...
    return {
        ...
        "vwap_distance_atr": (row["close"] - row["vwap"]) / row["atr"] if row["atr"] else 0,
        "body_size_atr": abs(row["close"] - row["open"]) / row["atr"] if row["atr"] else 0,
        "range_position": row.get("range_position", 0),  # optional if precomputed
    }